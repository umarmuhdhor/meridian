...
switch (subcommand) {
  ...
  case "candidates": {
    ...
    const { checkSmartWalletsOnPool } = await import("./persistence/smart-wallets.js");
    const { recallForPool } = await import("./persistence/pool-memory.js");
  ...
  case "lessons": {
    if (sub2 === "add") {
      ...
      const { addLesson } = await import("./persistence/lessons.js");
      addLesson(text, [], { pinned: false, role: null });
      out({ saved: true, rule: text, outcome: "manual", role: null });
    } else {
      const { listLessons } = await import("./persistence/lessons.js");
  ...
  case "pool-memory": {
    ...
    const { getPoolMemory } = await import("./persistence/pool-memory.js");
  ...
  case "evolve": {
    const { config } = await import("./config.js");
    const { evolveThresholds } = await import("./persistence/lessons.js");
  ...
  case "blacklist": {
    if (sub2 === "add") {
      ...
      const { addToBlacklist } = await import("./persistence/token-blacklist.js");
      out(addToBlacklist({ mint: flags.mint, reason: flags.reason }));
    } else if (sub2 === "list" || !sub2) {
      const { listBlacklist } = await import("./persistence/token-blacklist.js");
      out(listBlacklist());
    } else {
  ...
  // ── performance ──────────────────────────────────────────────────
  case "performance": {
    const { getPerformanceHistory, getPerformanceSummary } = await import("./persistence/lessons.js");
